...
class TestAnalyticDimensionCase(SavepointCase):
    ...
    def setUpClass(cls, *args, **kwargs):
        ...
        cls.analytic_account_id = cls.env.ref("analytic.analytic_absences").id
        ...
        cls.company_id = cls.env["res.users"].browse(cls.env.uid).company_id.id
    ...
    def test_dimension_changes(self):
        ...
        line = self.analytic_line_obj.create(values)
        with self.assertRaises(ValidationError):
            self.analytic_tag_concept_a.analytic_dimension_id = analytic_dimension_test
